 ...
 *
 * @param id       The internal holder ID that this config is referring to.
 * @param spawnMin The minimum holder value for natural spawned chickens.
 * @param spawnMax The maximum holder value for natural spawned chickens,
 *                 If min and max are both zero, then chickens will not spawn with this holder.
 * @param evoRate  Controls the rate that this holder will increase with breeding, (Also effected by breeding modifier)
 * @param evoLimit "Theoretical" holder value limit, though it won't actually be possible to reach this value.
 * @param evoExpo  The closer a holder value is to its evoLimit, the harder it is to progress further. Meaning its likely impossible to actually ever get to the 'evoLimit' due to diminishing returns.
 ...
 *                 The equation for evolution difficulty is: (traitValue / evoLimit) ^ evoExponent (Once the result of this equation reaches 1, no further holder progression is possible)
 ...
 */
public record TraitConfig(Trait trait, double spawnMin, double spawnMax, double evoRate, double evoLimit, double evoExpo) {
